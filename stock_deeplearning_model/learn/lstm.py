...
all=[]
#여러 기업 테스트 반복문으로
for input_file in glob.glob(os.path.join('/home/syl/tensor_stock_test/learn','data_*')):
    ...
    #상한가 종목 정보 1
    if predict > present:
        profit.append([company, present_price[-1], predict_price[-1], loss, 1])
    #하한가 종목 정보 0
    else:
        profit.append([company, present_price[-1], predict_price[-1], loss, 0])
...
#데이터 웹으로 보내기
#company/ present/ predict/ loss/ state
length = len(profit)
for i in range(0, length):
    url = sendStockGraphS3.uploadFile(profit[i][0])
    sendStockData.update(profit[i][0], profit[i][1], profit[i][2], profit[i][3], profit[i][4], url)
