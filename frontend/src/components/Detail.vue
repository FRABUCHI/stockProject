<template>
  <div class="detail">
    ...
    <br>
    <div class="table">
      <b-table style="max-width: 50rem" striped hover :fields="detail_fields" :items="stock"></b-table>
    </div>
    <b-card
    ...
    async getData() {
      ...
      await this.$http
        ...
        .then(res => {
          ...
          //오차수정작업
          this.stockUp.predict_price = Math.round(
            this.stock[0].close *
              (this.stockUp.predict_price / this.stockUp.present_price)
          );
          this.stockUp.predict_price = this.numberWithCommas(
            this.stockUp.predict_price
          );
